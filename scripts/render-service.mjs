...
export async function planEpisode(data, options = {}) {
  ...
  const longScenes = config.productionMode === "long-scenes";
  ...
  const system = longScenes ? longSystem : shortSystem;
  ...
  const minimumShotCount = narrationDuration ? Math.max(1, Math.ceil(narrationDuration / (longScenes ? 12 : 4))) : null;
  const targetShotCount = narrationDuration ? Math.max(minimumShotCount, longScenes ? Math.round(narrationDuration / targetClipDuration) : Math.ceil(narrationDuration / 3.3)) : null;
  ...
  const raw = await completeText(config, [{role:"system",content:system},{role:"user",content:JSON.stringify({script:config.script, contentFormat:config.contentFormat || "Documentary", visualStyle:config.visualStyle || "Photorealistic", creativeDirection:config.creativeDirection || "", productionMode:longScenes ? "long-scenes" : "short-shots", targetClipDurationSeconds:longScenes ? targetClipDuration : null, screenRatio, narrationDurationSeconds:narrationDuration || null, minimumShotCount, targetShotCount, maximumShotCount, localTranscriptionSegments:transcriptionSegments})}], { temperature:.25, maxTokens:8000, fetchImpl:options.fetchImpl });
  const parsed = await parseOrRepairProviderJson(raw, config, options, "an object with a shots array");
  return normalizePlannedShots(parsed.shots, Number(config.audioDuration) || 0, { contentFormat:config.contentFormat, visualStyle:config.visualStyle, creativeDirection:config.creativeDirection, productionMode:longScenes ? "long-scenes" : "short-shots", targetClipDuration, screenRatio, transcription:config.transcription });
}
