...
export default function StudioApp() {
  ...
  const [provider, setProvider] = useState<ProviderSettings>({
    ...
    apiKey: "", textKind:"openai", textEndpoint: "https://api.openai.com/v1/chat/completions", textModel: "gpt-4.1-mini", textApiKey: "",
  ...
  function persistProject(snapshot = projectSnapshot()) {
    ...
    try {
      const safeShots = (snapshot.shots as Shot[] || []).map((shot) => ({ ...shot, image:"", variants:[], status:shot.status === "generated" ? "planned" : shot.status }));
  ...
  async function analyze() {
    ...
    try {
      ...
      const planned:Shot[] = data.shots.map((shot:Partial<Shot>, index:number) => ({ ...shot, id:`shot-${Date.now()}-${index}`, index, status:"planned", locked:false, image:"", variants:[], provider:"", seed:"" })) as Shot[];
  ...
  async function requestShotImage(shot: Shot) {
    ...
    try {
      ...
      updateShot(shot.id, { image, variants: [...shot.variants, image], status: "generated", provider: provider.model });
      return { ok:true, shot };
    } catch (error) {
  ...
  async function renderVideo(target:"preview" | "download", resolution:string) {
    ...
    if (shots.some((shot) => !shot.image)) { setMessage("Generate every storyboard image before building the video."); setStage("storyboard"); return; }
  ...
  return (
    <main className="studio-shell">
      ...
      <section className="workspace">
        ...
        {stage === "storyboard" && <Storyboard shots={shots} selected={selected} setSelectedId={setSelectedId} updateShot={updateShot} generateOne={generateOne} generateAll={generateAll} totalDuration={totalDuration} busy={busy} />}
      ...
      <div className="statusbar"><span>{busy ? <><i className="spinner"/>{busy}</> : message}</span><span>{shots.length} shots · {formatTime(totalDuration)} · 9:16</span></div>
...
function Storyboard({ shots, selected, setSelectedId, updateShot, generateOne, generateAll, totalDuration, busy }: any) {
  ...
  return <div className="panel storyboard-panel"><div className="section-head compact"><div><span className="eyebrow">VISUAL PLAN</span><h1>Storyboard</h1><p>{shots.length} shots aligned across {formatTime(totalDuration)}</p></div><button className="primary" onClick={generateAll} disabled={!!busy}>{busy ? "Generating…" : "Generate all unlocked"}</button></div>
    <div className="story-grid"><div className="shot-list">{shots.map((shot: Shot) => <button key={shot.id} className={`shot-row ${selected?.id === shot.id ? "selected" : ""}`} onClick={() => setSelectedId(shot.id)}>
      <div className="thumb">{shot.image ? <img src={shot.image} alt="Generated shot"/> : <span>{String(shot.index + 1).padStart(2,'0')}</span>}</div><div className="shot-summary"><div><span className={`tag type-${shot.type.toLowerCase()}`}>{shot.type}</span><span className="time">{formatTime(shot.start)}—{formatTime(shot.end)}</span></div><p>{shot.narration}</p><small>{shot.duration.toFixed(1)}s · {shot.motion}</small></div><span className={`state state-${shot.status}`}>{shot.locked ? "Locked" : shot.status}</span></button>)}</div>
      {selected && <div className="inspector"><div className="phone-frame"><div className="phone-canvas">{selected.image ? <img src={selected.image} alt="Selected shot preview"/> : <div className="empty-visual"><span>{String(selected.index + 1).padStart(2,'0')}</span><b>Awaiting image</b></div>}<div className="preview-captions"><b>{selected.narration}</b><span>{selected.chinese}</span></div></div></div>
...
function ExportPanel({ shots, approved, duration, audioName, bgm, buildPreview, prepareDownload, previewUrl, downloadUrl, downloadResolution, setDownloadResolution, busy }: any) {
  const ready = Boolean(shots.length && audioName && shots.every((shot:Shot) => shot.image));
...
function Settings({ provider, setProvider, status, refreshStatus, close }: any) {
  ...
  async function testConnection(target:string) {
    ...
    try {
      const response = await fetch(`${SERVICE}/providers/test`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ target, ...provider, endpoint:target === "text" ? provider.textEndpoint : provider.endpoint, model:target === "text" ? provider.textModel : provider.model, apiKey:target === "text" ? provider.textApiKey : provider.apiKey }) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error || "Connection failed");
      setTestResult(`${target === "image" ? "Image" : "Translation"} provider connected.`); await refreshStatus();
