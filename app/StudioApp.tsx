...
import { buildSrt as buildSrtFile, hasTimedTranscription, subtitleFileName, transcriptionForShots } from "./lib/subtitles";
...
type ProviderSettings = {
  ...
  videoKind: "volcengine" | "dashscope" | "pixstag"; videoEndpoint: string; videoModel: string; videoApiKey: string; videoConcurrency: number; videoResolution: string;
...
type ProviderStatus = {
  ...
  speech: { configured:boolean; endpoint:string; model:string; voice:string; voices:string[]; language:string; speed:number; source:string };
};
...
export default function StudioApp() {
  ...
  const [headlinePosition, setHeadlinePosition] = useState(4);
  const [preBroadcastStyle, setPreBroadcastStyle] = useState<SubtitleStyle | null>(null);
  const [headlineStyle, setHeadlineStyle] = useState<HeadlineStyle>(DEFAULT_HEADLINE_STYLE);
  ...
  const [shortClipDuration, setShortClipDuration] = useState(15);
  const [scriptDuration, setScriptDuration] = useState(3);
  const [minimaxVoice, setMinimaxVoice] = useState("English_Trustworthy_Man");
  const [minimaxModel, setMinimaxModel] = useState("speech-2.8-hd");
  const [minimaxSpeed, setMinimaxSpeed] = useState(1);
  const [minimaxVoices, setMinimaxVoices] = useState<MiniMaxVoice[]>([]);
  const [minimaxVoiceLang, setMinimaxVoiceLang] = useState("all");
  const [minimaxVoiceGender, setMinimaxVoiceGender] = useState("all");
  const [genre, setGenre] = useState("documentary");
  const [doubaoSpeaker, setDoubaoSpeaker] = useState("zh_male_xuanyijieshuo_uranus_bigtts");
  const [doubaoSpeechRate, setDoubaoSpeechRate] = useState(-10);
  ...
  const [buildProgress, setBuildProgress] = useState<{ percent:number; stage:string } | null>(null);
  ...
  const [chosenCoverUrl, setChosenCoverUrl] = useState("");
  ...
  const [provider, setProvider] = useState<ProviderSettings>({
    ...
    apiKey: "", videoKind:"volcengine", videoEndpoint:"https://ark.cn-beijing.volces.com/api/v3/contents/generations/tasks", videoModel:"doubao-seedance-2-0-260128", videoApiKey:"", videoConcurrency:2, videoResolution:"1080p",
  ...
  const [providerStatus, setProviderStatus] = useState<ProviderStatus>({
    ...
    speech: { configured:true, endpoint:"http://localhost:8010/v1/audio/speech", model:"mlx-community/Kokoro-82M-bf16", voice:"af_heart", voices:[], language:"a", speed:1, source:"default" },
  });
  ...
  function applyProjectState(parsed:any, recoveryMessage = "") {
    ...
    setBroadcastMode(Boolean(parsed.broadcastMode));
    ...
    setCoverShotId(restoredCoverShotId);
    setChosenCoverUrl(String(parsed.chosenCoverUrl || parsed.shots?.find((shot:Shot) => shot.id === restoredCoverShotId)?.image || ""));
  ...
  async function recoverInvalidTranscription(parsed:any) {
    if (!parsed?.audioData || !parsed?.transcription || hasTimedTranscription(parsed.transcription)) return;
    const isStory = genreId(parsed.genre) === "story";
    setBusy("Repairing narration timing");
    try {
      const filename = parsed.audioName || "narration.mp3";
      const audioData = String(parsed.audioData).startsWith("data:")
        ? parsed.audioData
        : await fileToDataUrl(new File([await (await fetch(parsed.audioData)).blob()], filename));
      const response = await fetch(`${SERVICE}/audio/transcribe`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ language:isStory ? "zh" : provider.transcriptionLanguage, filename, audioData }) });
      const transcription = await response.json(); if (!response.ok) throw new Error(transcription.error || "Local transcription failed");
      if (!hasTimedTranscription(transcription)) throw new Error("Local transcription returned no usable timestamps");
      const audioDuration = Number(transcription.duration) || Number(parsed.audioDuration) || 0;
      setTranscription(transcription); setAudioDuration(audioDuration);
      await persistProject({ ...parsed, audioDuration, transcription });
      setMessage("Narration timing repaired from the local transcription.");
    } catch (error) {
      setMessage(`Narration timing could not be repaired: ${error instanceof Error ? error.message : "service unavailable"}`);
    } finally { setBusy(""); }
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (parsed) {
        ...
        void recoverInvalidTranscription(parsed);
      }
    })();
  ...
  useEffect(() => {
    ...
    if (cachedStyles?.subtitleStyle) setSubtitleStyle(normalizeSubtitleStyle(cachedStyles.subtitleStyle));
    if (cachedStyles?.headlineStyle) setHeadlineStyle(normalizeHeadlineStyle(cachedStyles.headlineStyle));
    if (Number.isFinite(Number(cachedStyles?.headlinePosition))) setHeadlinePosition(Number(cachedStyles.headlinePosition));
  }, [stage, episodeId]);

  useEffect(() => {
    if (!initialized.current || !allowSave.current || !episodeId) return;
    if (stage === "livestream") return;
    writeStyleCache(episodeId, { subtitleStyle, headlineStyle, headlinePosition, broadcastMode });
  }, [stage, episodeId, subtitleStyle, headlineStyle, headlinePosition, broadcastMode]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`${SERVICE}/audio/voices?provider=minimax`)
      .then((response) => response.json())
      .then((data) => {
        if (cancelled) return;
        // Always keep the full built-in catalog, then append any voices the
        // MiniMax voice-list API returns (e.g. user-cloned voices), de-duped.
        const remote = Array.isArray(data.voices) ? data.voices : [];
        const seen = new Set<string>();
        const merged = [...MINIMAX_VOICE_FALLBACK, ...remote].filter((v) => !seen.has(v.voice_id) && seen.add(v.voice_id));
        setMinimaxVoices(merged);
      })
      .catch(() => { if (!cancelled) setMinimaxVoices(MINIMAX_VOICE_FALLBACK); });
    return () => { cancelled = true; };
  }, []);
  ...
  function goToStage(nextStage:string) {
    ...
    setStage(nextStage);
    if (nextStage === "livestream") refreshEpisodeHistory();
  }
  ...
  async function generateDocumentaryScript() {
    touchProject();
    const topic = title.trim() || script.trim().split(/\s+/).slice(0, 6).join(" ");
    if (!topic) { setMessage("Enter a topic or title before generating a script."); return; }
    if (!provider.textApiKey && !providerStatus.text.configured) { setMessage("Configure a text AI provider before generating a script."); setSettingsOpen(true); return; }
    const chineseScript = getGenre(genre).primaryLanguage === "zh";
    setBusy(chineseScript ? "Generating Chinese script" : "Generating documentary script");
    try {
      const response = await fetch(`${SERVICE}${chineseScript ? "/text/generate-story-script" : "/text/generate-documentary-script"}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...textProviderPayload(),
          topic, duration: scriptDuration, genre,
      ...
      const scriptLength = chineseScript ? data.script.trim().replace(/\s/g, "").length : data.script.trim().split(/\s+/).length;
      setMessage(`Script generated · ${scriptLength} ${chineseScript ? "characters" : "words"} · ${scriptDuration} minutes`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Script generation failed"); }
    finally { setBusy(""); }
  }

  async function regenerateOpeningVisual() {
    touchProject();
    const opening = shots[0];
    if (!opening || !script.trim()) { setMessage("No opening shot or script to regenerate from."); return; }
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Opening hook failed"); }
    finally { setBusy(""); }
  }
  ...
  async function generateNarration() {
    ...
    const isStory = g.id === "story";
    if (!script.trim()) { setMessage(isStory ? "先添加中文文案，再生成配音。" : "Add the English narration script before generating speech."); return; }
    setBusy("Synthesizing narration");
    try {
      const endpoint = isStory ? "/audio/synthesize-doubao" : "/audio/synthesize-minimax";
      const payload = isStory
        ? { script, speaker:doubaoSpeaker, speechRate:doubaoSpeechRate }
        : { script, voice:minimaxVoice, model:minimaxModel, speed:minimaxSpeed };
      ...
      await attachSynthesizedNarration(result);
    } catch (error) {
      setMessage(`${isStory ? "豆包" : "MiniMax"} synthesis failed: ${error instanceof Error ? error.message : "Service unavailable"}`);
      setBusy("");
    }
  }

  async function attachSynthesizedNarration(result: { audioData:string; filename:string; mimeType:string; transcription:Transcription | null }) {
    touchProject();
    setPreviewUrl(""); setDownloadUrl("");
    setBusy("Loading narration timing");
    const filename = result.filename || "minimax-narration.mp3";
    let transcription = result.transcription && hasTimedTranscription(result.transcription) ? result.transcription : null;
    if (!transcription) {
      try {
        const response = await fetch(`${SERVICE}/audio/transcribe`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ endpoint:provider.transcriptionEndpoint, language:getGenre(genre).id === "story" ? "zh" : provider.transcriptionLanguage, filename, audioData:result.audioData }) });
        const recovered = await response.json(); if (!response.ok) throw new Error(recovered.error || "Local transcription failed");
        if (hasTimedTranscription(recovered)) transcription = recovered;
      } catch { /* Use shot timing only when no timestamped transcription is available. */ }
    }
    setAudioName(filename); setAudioData(result.audioData); setTranscription(transcription);
    setNarrationAutoplayRequest((current) => current + 1);
    let duration = Number(transcription?.duration) || 0;
    if (!duration) {
      try {
        const blob = await (await fetch(result.audioData)).blob();
        duration = await readAudioDuration(new File([blob], filename, { type: result.mimeType || "audio/mp3" }));
      } catch { /* keep 0 */ }
    }
    setAudioDuration(duration);
    persistProject(projectSnapshot({ audioName: filename, audioData: result.audioData, audioDuration: duration, transcription }));
    const providerName = getGenre(genre).id === "story" ? "豆包" : "MiniMax";
    setMessage(transcription
      ? `${providerName} narration ready: ${transcription.segments?.length || 0} timed segments · ${formatTime(duration)}`
      : `${providerName} narration ready (no subtitle file returned).`);
    setBusy("");
  }
  ...
  async function handleShotImageUpload(shot: Shot, file: File) {
    touchProject();
    setBusy("Uploading image");
    try {
      const dataUrl = await fileToDataUrl(file);
      const response = await fetch(`${SERVICE}/image/upload`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ image:dataUrl, screenRatio, episodeId, episodeTitle:title, assetName:shot.id }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Upload failed");
      updateShot(shot.id, { image:data.image, status:"generated", imageStatus:"generated", imageError:"", provider:"upload", video:"", videoStatus:"idle", videoError:"", videoProvider:"" });
      setMessage("Image uploaded and applied to shot.");
    } catch (error) {
  ...
  async function requestShotVideo(shot:Shot) {
    ...
    try {
      const response = await fetch(`${SERVICE}/video/generate`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ videoKind:provider.videoKind, endpoint:provider.videoEndpoint, model:provider.videoModel, apiKey:provider.videoApiKey, resolution:provider.videoResolution, generationMode:productionMode, videoPrompt:shot.videoPrompt, image:directTextToVideo ? "" : shot.image, motion:shot.motion, duration:shot.duration, screenRatio, episodeId, episodeTitle:title, assetName:shot.id }) });
  ...
  async function translateAll() {
    if (isStory) return; // 中文单语模式无需生成翻译字幕
    touchProject();
  ...
  async function renderVideo(resolution:string) {
    ...
    const renderJobId = typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `render-${Date.now()}`;
    setBuildProgress({ percent:0, stage:"Queued" });
    const progressTimer = setInterval(async () => {
      try {
        const statusResponse = await fetch(`${SERVICE}/render/jobs/${encodeURIComponent(renderJobId)}`);
        if (!statusResponse.ok) return;
        const statusData = await statusResponse.json();
        if (statusData?.job) setBuildProgress({ percent:Number(statusData.job.percent) || 0, stage:String(statusData.job.stage || "") });
    ...
    finally { clearInterval(progressTimer); setBuildProgress(null); setBusy(""); }
  }

  async function renderSampleVideo(resolution:string) {
    if (!audioData) { setMessage("Upload the recorded narration before building the video."); setStage("episode"); return; }
    if (shots.length < 2) { setMessage("Need at least 2 shots to build a sample."); return; }
    const sampleShots = shots.slice(0, 2);
    if (!visualCoverage(sampleShots).complete) { setMessage("The first 2 shots need either a generated image or a video clip before building."); goToStage("storyboard"); return; }
    const preset = videoResolution(resolution, screenRatio);
    setPreviewUrl(""); setDownloadUrl("");
    setBusy(`Building ${preset.label} sample (first 2 shots)`);
    const renderJobId = typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `render-${Date.now()}`;
    setBuildProgress({ percent:0, stage:"Queued" });
    const progressTimer = setInterval(async () => {
      try {
    ...
    try {
      const sampleTranscription = transcriptionForShots(transcription, sampleShots);
      const response = await fetch(`${SERVICE}/render`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ episodeId, title, renderJobId, shots:sampleShots, narrationData:audioData, transcription:sampleTranscription, timelineDuration:Number(sampleTranscription?.duration) || 0, voicePreset:denoiseNarration ? "denoise" : "original", bgmPath:bgm, bgmVolume:bgmVolume / 100, subtitleStyle, broadcastMode, headlineText, headlinePosition, width:preset.width, height:preset.height }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "Render failed");
      const sampleDuration = sampleShots.reduce((sum, shot) => sum + shot.duration, 0);
      const url = `${SERVICE}${data.url}`;
      const buildRecord:VideoBuild = { id:String(data.id || `build-${Date.now()}`), path:String(data.url || ""), url, screenRatio, resolution:String(resolution), width:preset.width, height:preset.height, duration:Number(data.duration) || sampleDuration, createdAt:Date.now() };
      setVideoBuilds((current) => [buildRecord, ...current.filter((build) => build.id !== buildRecord.id)]);
      setPreviewUrl(url); setDownloadUrl(url);
      setMessage(`${preset.label} ${screenRatio} sample built in ${data.seconds.toFixed(1)} seconds, saved to build history, and ready to download.`);
    } catch (error) { setMessage(error instanceof Error ? `${error.message}. Start the local render service with "npm run render-service".` : "Render failed"); }
    finally { clearInterval(progressTimer); setBuildProgress(null); setBusy(""); }
  }

  function deleteBuild(id:string) {
    setVideoBuilds((current) => current.filter((build) => build.id !== id));
    touchProject();
  }
  ...
  async function generateCover() {
    ...
    try {
      ...
      let data:any;
      try { data = await response.json(); } catch { throw new Error(`Cover generation failed (HTTP ${response.status})`); }
      if (!response.ok) {
        const msg = typeof data?.error === "string" ? data.error : data?.error?.message || data?.message || `Cover generation failed (HTTP ${response.status})`;
        throw new Error(msg);
      }
  ...
  async function saveCover(cover:CoverImage) {
    ...
    try {
      ...
      const response = await fetch(`${SERVICE}/covers/bake`, {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body:JSON.stringify({ episodeId, title, image, screenRatio }),
      });
      const saved = await response.json();
      if (!response.ok) throw new Error(saved.error || "Could not save the cover");
      const nextCover:CoverImage = { ...saved, prompt:cover.prompt || coverPrompt, provider:"manual", createdAt:Number(saved.createdAt) || Date.now() };
      setCovers((current) => [nextCover, ...current.filter((item) => item.id !== nextCover.id)]);
  ...
  return (
    <main className={`studio-shell ${audioData && !episodesOpen && stage !== "prompter" ? "has-narration-bar" : ""}`}>
      ...
      <section className={`workspace${stage === "prompter" ? " workspace-prompter" : ""}`}>
        {stage !== "livestream" && stage !== "prompter" && <header className="topbar">
...
function NarrationBar({ audioData, audioName, audioDuration, autoplayRequest, previewShot, continuous, shots, setSelectedId, setPreviewActive, onTimeUpdate }: any) {
...
function EpisodePanel({ title, setTitle, script, setScript, contentFormat, setContentFormat, visualStyle, setVisualStyle, creativeDirection, setCreativeDirection, productionMode, setProductionMode, longClipDuration, setLongClipDuration, shortClipDuration, setShortClipDuration, mode, setMode, touchProject, audioName, transcription, handleAudio, generateNarration, minimaxVoice, setMinimaxVoice, minimaxModel, setMinimaxModel, minimaxSpeed, setMinimaxSpeed, minimaxVoices, minimaxVoiceLang, setMinimaxVoiceLang, minimaxVoiceGender, setMinimaxVoiceGender, genre, setGenre, doubaoSpeaker, setDoubaoSpeaker, doubaoSpeechRate, setDoubaoSpeechRate, analyze, generateDocumentaryScript, scriptDuration, setScriptDuration, busy }: any) {
  const availableVoices = minimaxVoices.length ? minimaxVoices : MINIMAX_VOICE_FALLBACK;
  const currentGenre = getGenre(genre);
  const isStory = currentGenre.id === "story";
  // The episode editor does not offer a free-form custom voice_id input, so the
  // "custom" placeholder is dropped from the picker here (the digital-human app
  // keeps it behind its own custom-voice field).
  const voiceOptions = filterVoices(availableVoices, minimaxVoiceLang, minimaxVoiceGender).filter((v: MiniMaxVoice) => v.type !== "custom");
  return <div className="panel intake-panel"><div className="section-head"><div><span className="eyebrow">SOURCE MATERIAL</span><h1>Create an episode</h1><p>Add the finished script, then generate AI narration or attach a recording. Choose quick image-led shots or longer text-to-video scenes.</p></div><button className="primary large" onClick={analyze} disabled={!!busy}>{busy || "Analyze with AI"}</button></div>
    <div className="intake-grid"><label className="field episode-title-field"><span>Episode title</span><input value={title} placeholder="Give this episode a recognizable title…" onChange={(e) => { touchProject(); setTitle(e.target.value); }} autoComplete="off"/></label><label className="field span-2"><span>{currentGenre.scriptLabel}</span><div className="script-toolbar"><select className="duration-select" value={scriptDuration} onChange={(e) => { touchProject(); setScriptDuration(Number(e.target.value)); }} aria-label="Script duration"><option value={2}>2 min</option><option value={3}>3 min</option><option value={4}>4 min</option><option value={5}>5 min</option><option value={6}>6 min</option></select><button type="button" className="secondary" onClick={generateDocumentaryScript} disabled={!!busy}>{busy === "Generating documentary script" || busy === "Generating Chinese script" ? "Generating…" : "Generate script"}</button></div><textarea value={script} placeholder={currentGenre.scriptPlaceholder} onChange={(e) => { touchProject(); setScript(e.target.value); }} rows={13}/><small>{script.trim() ? (currentGenre.primaryLanguage === "zh" ? script.trim().replace(/\s/g, "").length : script.trim().split(/\s+/).length) : 0} {currentGenre.primaryLanguage === "zh" ? "characters" : "words"}</small></label>
      <div className="stack"><label className="field"><span>视频类型</span><select value={genre} onChange={(e) => { const next = e.target.value; const ng = getGenre(next); touchProject(); setGenre(next); setContentFormat(ng.defaultContentFormat); setVisualStyle(ng.defaultVisualStyle); }}>{GENRE_LIST.map((item) => <option key={item.id} value={item.id}>{item.labelZh}</option>)}</select></label><label className="field"><span>Content format</span><select value={contentFormat} onChange={(e) => { touchProject(); setContentFormat(e.target.value); }}><option>Documentary</option><option>Educational explainer</option><option>Narrative story</option><option>News recap</option><option>Product story</option><option>History documentary</option><option>Other</option></select></label><label className="field"><span>Visual style</span><select value={visualStyle} onChange={(e) => { touchProject(); setVisualStyle(e.target.value); }}><option>Photorealistic</option><option>Cinematic illustration</option><option>Editorial collage</option><option>3D animation</option><option>Anime</option><option>Minimal graphic</option></select></label><label className="field"><span>Creative direction</span><input value={creativeDirection} placeholder="Audience, mood, setting, visual constraints…" onChange={(e) => { touchProject(); setCreativeDirection(e.target.value); }}/></label><label className="field workflow-field"><span>Visual workflow</span><select value={productionMode} onChange={(e) => { touchProject(); setProductionMode(e.target.value); }}><option value="short-shots">Dynamic short shots · image first</option><option value="mixed">Mixed · all images + selected videos</option><option value="long-scenes">Long scenes · direct text to video</option></select><small>{productionMode === "long-scenes" ? "No storyboard images are generated." : productionMode === "mixed" ? "All shots get images; about one in four is selected for video." : "Create images, then optionally animate them."}</small></label>{productionMode === "short-shots" && <label className="field clip-length-field"><span>Target shot length</span><div><input aria-label="Target short-shot length" type="range" min="5" max="20" step="1" value={shortClipDuration} onChange={(e) => { touchProject(); setShortClipDuration(Number(e.target.value)); }}/><output>{shortClipDuration}s</output></div><small>Adjustable from 5–20 seconds; longer shots mean fewer images to generate.</small></label>}{productionMode === "long-scenes" && <label className="field clip-length-field"><span>Target clip length</span><div><input aria-label="Target long-scene clip length" type="range" min="6" max="12" step="1" value={longClipDuration} onChange={(e) => { touchProject(); setLongClipDuration(Number(e.target.value)); }}/><output>{longClipDuration}s</output></div><small>Adjustable from 6–12 seconds; scene boundaries follow natural pauses.</small></label>}<label className="field"><span>Generation mode</span><select value={mode} onChange={(e) => { touchProject(); setMode(e.target.value); }}><option>Plan only</option><option>Review then batch</option><option>Fully automatic</option></select></label>
        {isStory ? <section className="speech-card"><div className="speech-card-head"><div><span>AI 配音</span><b>豆包 TTS</b><small>悬疑解说 · seed-tts-2.0</small></div><button type="button" className="primary" onClick={generateNarration} disabled={!!busy || !script.trim()}>{busy === "Synthesizing narration" ? "生成中…" : audioName ? "重新生成配音" : "生成配音"}</button></div><div className="speech-controls"><label className="field"><span>音色</span><select value={doubaoSpeaker} onChange={(event) => { touchProject(); setDoubaoSpeaker(event.target.value); }}><option value="zh_male_xuanyijieshuo_uranus_bigtts">悬疑解说（沉稳男声）</option><option value="zh_male_cixingjieshuonan_uranus_bigtts">磁性解说男声</option><option value="zh_male_shenyeboke_uranus_bigtts">深夜播客</option><option value="zh_male_gaolengchenwen_uranus_bigtts">高冷沉稳</option></select></label><label className="field speech-speed"><span>语速</span><div><input aria-label="Doubao speech rate" type="range" min="-30" max="20" step="5" value={doubaoSpeechRate} onChange={(event) => { touchProject(); setDoubaoSpeechRate(Number(event.target.value)); }}/><output>{doubaoSpeechRate}</output></div></label></div></section> : <section className="speech-card"><div className="speech-card-head"><div><span>AI narration</span><b>MiniMax TTS</b><small>{availableVoices.find((v: MiniMaxVoice) => v.voice_id === minimaxVoice)?.name || minimaxVoice} · {minimaxModel}</small></div><button type="button" className="primary" onClick={generateNarration} disabled={!!busy || !script.trim()}>{busy === "Synthesizing narration" ? "Generating…" : audioName ? "Regenerate narration" : "Generate narration"}</button></div><div className="speech-controls"><label className="field"><span>Voice</span><select value={minimaxVoice} onChange={(event) => { touchProject(); setMinimaxVoice(event.target.value); }}>{!voiceOptions.some((v: MiniMaxVoice) => v.voice_id === minimaxVoice) && <option value={minimaxVoice}>{minimaxVoice}</option>}{voiceOptions.map((v: MiniMaxVoice) => <option key={v.voice_id} value={v.voice_id}>{v.name}{v.language ? ` · ${v.language}` : ""}</option>)}</select></label><label className="field"><span>Model</span><select value={minimaxModel} onChange={(event) => { touchProject(); setMinimaxModel(event.target.value); }}>{MINIMAX_TTS_MODELS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}</select></label><label className="field speech-speed"><span>Speed</span><div><input aria-label="Narration speech speed" type="range" min=".75" max="1.35" step=".05" value={minimaxSpeed} onChange={(event) => { touchProject(); setMinimaxSpeed(Number(event.target.value)); }}/><output>{Number(minimaxSpeed).toFixed(2)}×</output></div></label></div><label className="field"><span>Language</span><select value={minimaxVoiceLang} onChange={(event) => setMinimaxVoiceLang(event.target.value)}><option value="all">All languages</option>{Object.entries(LANG_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></label><label className="field"><span>Gender</span><select value={minimaxVoiceGender} onChange={(event) => setMinimaxVoiceGender(event.target.value)}><option value="all">All genders</option>{Object.entries(GENDER_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></label></section>}
        <div className="narration-choice"><span>or attach a recording</span></div><label className="upload-card mini"><input type="file" accept="audio/*" onChange={handleAudio}/><b>{audioName ? "Replace narration audio" : "Upload narration audio"}</b><span>{audioName || "MP3, WAV, M4A or AAC · transcribed locally"}</span></label>{audioName && <div className={`transcript-note ${transcription ? "ready" : ""}`}><b>{transcription ? "Transcript ready" : "Waiting for transcript"}</b><span>{transcription ? `${transcription.segments.length} timed segments · ${formatTime(transcription.duration)}` : "MiniMax returns timed subtitles with the narration, or upload audio for local transcription."}</span></div>}</div></div></div>;
}
...
function SubtitleOverlay({ shot, shots, subtitleStyle, audioElapsed, transcription, activeChunk, isStory = false }:any) {
  const style = normalizeSubtitleStyle(subtitleStyle);
  const outlinePx = style.outline || 0;
  const outlineColor = style.backgroundColor;
  const shadow = outlinePx > 0
    ? [`0 ${outlinePx}px 0 ${outlineColor}`,`0 -${outlinePx}px 0 ${outlineColor}`,`${outlinePx}px 0 0 ${outlineColor}`,`-${outlinePx}px 0 0 ${outlineColor}`,`${outlinePx}px ${outlinePx}px 0 ${outlineColor}`,`-${outlinePx}px ${outlinePx}px 0 ${outlineColor}`,`${outlinePx}px -${outlinePx}px 0 ${outlineColor}`,`-${outlinePx}px -${outlinePx}px 0 ${outlineColor}`].join(", ")
    : "none";
...
function BroadcastHeadlineOverlay({ headlineText, headlinePosition, subtitleStyle, headlineStyle, subHeadlineText, subHeadlineStyle }:any) {
  ...
  const [r, g, b] = [1, 3, 5].map((i) => Number.parseInt((hs?.bgColor || "#000000").slice(i, i + 2), 16));
  const bgAlpha = (hs?.bgOpacity ?? 65) / 100;
...
function SubtitleStyleControls({ subtitleStyle, setSubtitleStyle, broadcastMode, setBroadcastMode, headlineText, setHeadlineText, headlinePosition, setHeadlinePosition, preBroadcastStyle, setPreBroadcastStyle, hideBroadcastToggle, hideHeadlineRow, isStory }:any) {
  ...
  return <section className="subtitle-style-dock" aria-label="Subtitle style"><div className="subtitle-dock-title"><span className="eyebrow">SUBTITLES</span><b>Style</b></div><div className="subtitle-dock-body">
    {(broadcastMode || hideBroadcastToggle) && !hideHeadlineRow && <div className="subtitle-dock-headline-row"><label className="field headline-field"><span>Headline</span><input type="text" value={headlineText} onChange={(event) => setHeadlineText(event.target.value)} placeholder="Broadcast headline text…"/></label><label className="dock-range"><span>HL pos</span><input aria-label="Headline top position" type="range" min="0" max="100" step="1" value={headlinePosition} onChange={(event) => setHeadlinePosition(Number(event.target.value))}/><output>{headlinePosition}%</output></label></div>}
    ...
    <label className="dock-range stepped"><span>Position</span><div className="livestream-stepper dock-stepper"><button type="button" aria-label="Move subtitles up" onClick={()=>setSubtitleStyle({ position:Math.max(5, subtitleStyle.position - 5) })}>−</button><input aria-label="Subtitle bottom position" type="range" min="5" max={positionMax} step="1" value={subtitleStyle.position} onChange={(event)=>setSubtitleStyle({ position:Number(event.target.value) })}/><button type="button" aria-label="Move subtitles down" onClick={()=>setSubtitleStyle({ position:Math.min(positionMax, subtitleStyle.position + 5) })}>+</button></div><output>{subtitleStyle.position}%</output></label>
    <label className="dock-color" title={isStory ? "中文文字颜色" : "English text color"}><span>{isStory ? "中文" : "EN"}</span><input aria-label="Primary subtitle color" type="color" value={subtitleStyle.englishColor} onChange={(event)=>setSubtitleStyle({ englishColor:event.target.value })}/></label>
    {!isStory && <label className="dock-color" title="Chinese text color"><span>中文</span><input aria-label="Chinese subtitle color" type="color" value={subtitleStyle.chineseColor} onChange={(event)=>setSubtitleStyle({ chineseColor:event.target.value })}/></label>}
...
function ExportPanel({ title, productionMode, shots, approved, duration, audioName, bgm, build, buildSample, subtitleStyle, broadcastMode, headlineText, headlinePosition, previewUrl, downloadUrl, videoBuilds, deleteBuild, downloadResolution, setDownloadResolution, screenRatio, busy, buildProgress, transcription, genre }: any) {
  ...
  const sampleReady = Boolean(sampleCoverage.complete && audioName);
...
function CoverStudio({ defaultHeadline, coverHeadline, setCoverHeadline, coverTitlePosition, setCoverTitlePosition, coverTitleVertical, setCoverTitleVertical, coverTitleScale, setTitleScale, coverTitleWidth, setCoverTitleWidth, coverPrompt, setCoverPrompt, suggestedCoverPrompt, covers, shots, coverShotId, setCoverShotId, chosenCoverUrl, setChosenCoverUrl, generateCover, downloadCover, saveAndDownloadCover, screenRatio, busy, coverPreviewRef }:any) {
  ...
  return <section className={`cover-studio ratio-${screenRatio.replace(":","-")}`}>
<div className="build-library-head">
<div>
<span className="eyebrow">VIDEO COVER</span>
<h2>{currentCover ? "Place the cover title" : "Generate cover artwork"}</h2>
</div>
<span>{SCREEN_RATIOS[screenRatio as keyof typeof SCREEN_RATIOS]?.label || screenRatio} · {screenRatio}</span>
</div>
<div className="cover-studio-grid">
<div ref={coverPreviewRef} className={`cover-preview ${currentCover ? `has-cover title-${coverTitlePosition}` : ""}`} style={{ aspectRatio:screenRatio.replace(":"," / ") }}>{currentCover ? <>
<img src={currentCover.url} alt={`Generated ${screenRatio} video cover`}/>
...
<div className="cover-preview-copy" style={{ left:titleInset, right:titleInset, top:`${coverTitleVertical}%`, bottom:"auto", transform:"translateY(-50%)" }}>
<i/>
<strong style={{ fontSize:`calc(8.5cqw * ${coverTitleScale / 100})` }}>{headline}</strong>
</div>
...
</label>{currentCover && <div className="cover-title-editor">
<label className="field cover-headline-field">
<span>Cover headline</span>
<input maxLength={90} value={coverHeadline} placeholder={defaultHeadline || "Add an attention-grabbing headline"} onChange={(event) => setCoverHeadline(event.target.value)}/>
<small>Keep it short and specific. The episode title is used when this field is empty.</small>
</label>
<fieldset className="cover-position-field">
<legend>Title position</legend>
<div>{COVER_TITLE_POSITIONS.map((option) => <button type="button" key={option.id} className={coverTitlePosition === option.id ? "chosen" : ""} title={option.label} aria-label={option.label} aria-pressed={coverTitlePosition === option.id} onClick={() => setCoverTitlePosition(option.id)}>
<i/>
</button>)}</div>
...
<div>
<input aria-label="Cover title vertical position" type="range" min="2" max="92" step="1" value={coverTitleVertical} onChange={(e) => setCoverTitleVertical(Number(e.target.value))}/>
<output>{coverTitleVertical}%</output>
</div>
...
<div>
<input aria-label="Cover title font size scale" type="range" min="50" max="200" step="5" value={coverTitleScale} onChange={(e) => setTitleScale(Number(e.target.value))}/>
<output>{coverTitleScale}%</output>
</div>
...
<div>
<input aria-label="Cover title text width" type="range" min="50" max="95" step="1" value={coverTitleWidth} onChange={(e) => setCoverTitleWidth(Number(e.target.value))}/>
<output>{coverTitleWidth}%</output>
</div>
...
<img src={cover.url} alt={`${cover.screenRatio} cover generated ${cover.createdAt ? new Date(cover.createdAt).toLocaleString() : ""}`}/>
...
function LivestreamPage({ shots, audioData, covers, chosenCoverUrl: initialChosenCoverUrl, transcription: initialTranscription, subtitleStyle, setSubtitleStyle, broadcastMode, setBroadcastMode, headlineText, setHeadlineText, headlinePosition, setHeadlinePosition, headlineStyle, setHeadlineStyle, preBroadcastStyle, setPreBroadcastStyle, episodeHistory, loadEpisodeData, currentEpisodeId }: any) {
  ...
  const [selectedEpisodeId, setSelectedEpisodeId] = useState(currentEpisodeId);
  ...
  const defaultBackground = lsChosenCoverUrl || livestreamCovers.find((c: CoverImage) => c.screenRatio === livestreamRatio)?.url || livestreamCovers[0]?.url || "";
  ...
  useEffect(() => {
    const cached = readStyleCache(currentEpisodeId);
    if (cached) {
      if (cached.subtitleStyle) setSubtitleStyle(cached.subtitleStyle);
      if (cached.headlineStyle) setHeadlineStyle(cached.headlineStyle);
      if (Number.isFinite(Number(cached.headlinePosition))) setHeadlinePosition(Number(cached.headlinePosition));
      if (["9:16", "9:16c", "16:9"].includes(cached.livestreamRatio)) setLivestreamRatio(cached.livestreamRatio);
      if (cached.bgMode === "cover" || cached.bgMode === "shots") setBgMode(cached.bgMode);
      if (Number.isFinite(Number(cached.videoPosition))) setVideoPosition(Math.max(0, Math.min(100, Number(cached.videoPosition))));
      if (Number.isFinite(Number(cached.repeatCount))) setRepeatCount(Math.max(1, Math.min(10, Number(cached.repeatCount))));
      if (typeof cached.autoPlay === "boolean") setAutoPlay(cached.autoPlay);
      if (Number.isFinite(Number(cached.playbackRate))) setPlaybackRate(Number(cached.playbackRate));
      if (typeof cached.prompterVisible === "boolean") setPrompterVisible(cached.prompterVisible);
      if (Number.isFinite(Number(cached.prompterVisibleCount))) setPrompterVisibleCount(Math.max(1, Math.min(8, Number(cached.prompterVisibleCount))));
      if (cached.subHeadlineStyle && typeof cached.subHeadlineStyle === "object") setSubHeadlineStyle(cached.subHeadlineStyle);
      if (typeof cached.subHeadlineText === "string") setSubHeadlineText(cached.subHeadlineText);
    }
  ...
  useEffect(() => {
    ...
    writeStyleCache(currentEpisodeId, { subtitleStyle, headlineStyle, headlinePosition, broadcastMode, livestreamRatio, bgMode, videoPosition, repeatCount, autoPlay, playbackRate, prompterVisible, prompterVisibleCount, subHeadlineText, subHeadlineStyle });
  }, [optionsHydrated, currentEpisodeId, subtitleStyle, headlineStyle, headlinePosition, broadcastMode, livestreamRatio, bgMode, videoPosition, repeatCount, autoPlay, playbackRate, prompterVisible, prompterVisibleCount, subHeadlineText, subHeadlineStyle]);
  ...
  useEffect(() => {
    if (readStyleCache(currentEpisodeId)) return;
  ...
  useEffect(() => {
    function handleSpaceToggle(event: KeyboardEvent) {
      ...
      if (!audioRef.current || !livestreamAudio || !(backgroundImage || (bgMode === "shots" && livestreamShots.length > 0))) return;
      event.preventDefault();
      togglePlayback();
    }
  ...
  const currentShot = livestreamShots.find((shot: Shot) => currentTime >= shot.start && currentTime < shot.end) || null;
  ...
  const prompterStyle = normalizeSubtitleStyle(subtitleStyle);
  const prompterOutline = prompterStyle.outline || 0;
  const prompterShadow = prompterOutline > 0
    ? [`0 ${prompterOutline}px 0 ${prompterStyle.backgroundColor}`,`0 -${prompterOutline}px 0 ${prompterStyle.backgroundColor}`,`${prompterOutline}px 0 0 ${prompterStyle.backgroundColor}`,`-${prompterOutline}px 0 0 ${prompterStyle.backgroundColor}`].join(", ")
    : "none";
  ...
  async function switchToEpisode(id: string) {
    ...
    setPlayCount(0);
    const data = await loadEpisodeData(id);
    if (data) {
...
{prompterVisible && prompterChunks.length > 0 && (
  <div className="livestream-prompter-overlay">
    <div className="livestream-prompter-chunks" style={{ marginBottom: `${prompterStyle.position}%`, background: subtitleCssBackground(prompterStyle) }}>
      {prompterChunks.slice(scrollOffset, scrollOffset + prompterVisibleCount).map((chunk, i) => {
        ...
        return (
          <div key={globalIndex} className={`livestream-prompter-chunk${isActive ? " active" : ""}${isPast ? " past" : ""}`} style={{ fontFamily: prompterStyle.fontFamily, fontWeight: prompterStyle.bold ? 700 : 400, textAlign: prompterStyle.alignment, borderLeftColor: isActive ? prompterStyle.englishColor : "transparent" }}>
            <div className="livestream-prompter-en" style={{ color: prompterStyle.englishColor, fontSize: `${prompterStyle.fontScale * 0.028}cqh`, fontWeight: "inherit", textShadow: prompterShadow }}>
              {chunk.english || " "}
            </div>
            {chunk.chinese && (
              <div className="livestream-prompter-zh" style={{ color: prompterStyle.chineseColor, fontSize: `${prompterStyle.fontScale * 0.022}cqh`, fontWeight: "inherit", textShadow: prompterShadow }}>
                {chunk.chinese}
              </div>
            )}
          </div>
      ...
      <div className="livestream-controls">
        ...
        <div className="livestream-bg-section">
          ...
          <div className="livestream-bg-options">
            ...
            {bgMode === "cover" && <>
            <div className="livestream-inline-row">
              <label className="field"><span>From covers</span><select value={backgroundImage} onChange={(event) => handleCoverSelect(event.target.value)}><option value="">Choose a cover…</option>{lsChosenCoverUrl && <option value={lsChosenCoverUrl}>Selected storyboard shot</option>}{livestreamCovers.map((cover: CoverImage) => <option key={cover.id} value={cover.url}>{cover.screenRatio} · {new Date(cover.createdAt).toLocaleString([], { dateStyle: "short", timeStyle: "short" })}</option>)}</select></label>
        ...
        <div className="livestream-bg-section">
          ...
          <div className="livestream-bg-options">
            ...
            <div className="livestream-color-swatches">
              <span>Text</span>
              {["#ffffff","#ffeb3b","#ff9800","#ff5252","#4caf50","#2196f3","#9c27b0","#000000"].map((c) => <button key={"t"+c} type="button" className={`swatch ${headlineStyle.textColor === c ? "active" : ""}`} style={{ background:c }} onClick={() => setHeadlineStyle({ ...headlineStyle, textColor: c })} aria-label={`Text color ${c}`}/>)}
            </div>
            <div className="livestream-color-swatches">
              <span>BG</span>
        ...
        <SubtitleStyleEditor subtitleStyle={subtitleStyle} setSubtitleStyle={setSubtitleStyle} broadcastMode={broadcastMode} setBroadcastMode={setBroadcastMode} headlineText={headlineText} setHeadlineText={setHeadlineText} headlinePosition={headlinePosition} setHeadlinePosition={setHeadlinePosition} preBroadcastStyle={preBroadcastStyle} setPreBroadcastStyle={setPreBroadcastStyle} lineCount={prompterVisibleCount} setLineCount={setPrompterVisibleCount} displayMode={prompterVisible ? "prompter" : "subtitle"} setDisplayMode={(mode) => setPrompterVisible(mode === "prompter")} hideBroadcastToggle hideHeadlineRow />
      </div>
    </div>
  </div>;
}
...
function Settings({ provider, setProvider, status, refreshStatus, close }: any) {
  ...
  return <div className="modal-backdrop" onMouseDown={(e)=>{if(e.target===e.currentTarget)close();}}><div className="modal"><div className="modal-head"><div><span className="eyebrow">LOCAL CONFIGURATION</span><h2>AI providers</h2></div><button onClick={close} aria-label="Close settings">×</button></div><p>For durable setup, add keys to <code>.env.local</code> and restart the app. Fields below are optional session-only overrides.</p>
    ...
    <label className="field"><span>Image provider</span><select value={imageProviderChoice} onChange={(e)=>chooseImageProvider(e.target.value)}><option value="openai">OpenAI-compatible images API</option><option value="volcengine">Volcengine Ark · Seedream · pay as you go</option><option value="volcengine-plan">Volcengine Agent Plan · Seedream</option><option value="dashscope">Aliyun Bailian · DashScope · Qwen-Image</option><option value="sdwebui">Local Stable Diffusion WebUI</option></select></label><label className="field"><span>Endpoint or API base URL</span><input value={provider.endpoint} onChange={(e)=>set({endpoint:e.target.value})}/></label><div className="two-fields"><label className="field"><span>Model</span><input value={provider.model} placeholder={provider.kind === "volcengine" ? "Ark Seedream model name or ID" : provider.kind === "dashscope" ? "e.g. qwen-image-3.0-pro" : "Model ID"} onChange={(e)=>set({model:e.target.value})}/></label><label className="field"><span>Session API key</span><input type="password" value={provider.apiKey} placeholder={status.image.configured ? "Loaded securely from .env.local" : "Optional session override"} onChange={(e)=>set({apiKey:e.target.value})}/></label></div><label className="field"><span>Parallel image jobs</span><input type="number" min="1" max="6" step="1" value={provider.imageConcurrency} onChange={(e)=>set({imageConcurrency:Math.max(1,Math.min(6,Number(e.target.value)||1))})}/><small>3 is recommended. Lower this if your provider reports rate limits.</small></label><button className="ghost test-button" onClick={()=>testConnection("image")} disabled={!!testing}>{testing === "image" ? "Testing…" : "Test image provider"}</button>
    <hr/><div className={`provider-status ${status.video?.configured ? "connected" : ""}`}><span>{status.video?.configured ? "●" : "○"}</span><div><b>Video generation{status.video?.kind === "dashscope" ? " · Aliyun Bailian Wan" : status.video?.kind === "pixstag" ? " · PixStag MiniMax-H3" : " · Volcengine Ark"}</b><small>{status.video?.configured ? `${status.video.model} · key loaded from .env.local` : "Required to turn storyboard images into video clips"}</small></div></div><label className="field"><span>Video provider</span><select value={videoProviderChoice} onChange={(e)=>chooseVideoProvider(e.target.value)}><option value="api">Volcengine Ark · pay-as-you-go API</option><option value="plan">Volcengine Ark · Agent Plan subscription</option><option value="dashscope">Aliyun Bailian · DashScope · Wan</option><option value="pixstag">PixStag · MiniMax-H3</option></select></label><label className="field"><span>Video task endpoint or API base URL</span><input value={provider.videoEndpoint} placeholder={videoProviderChoice === "dashscope" ? "https://dashscope.aliyuncs.com or your maas.aliyuncs.com host" : videoProviderChoice === "pixstag" ? "https://pixstag.com" : undefined} onChange={(e)=>set({videoEndpoint:e.target.value})}/></label><div className="two-fields"><label className="field"><span>{videoProviderChoice === "dashscope" ? "Wan model" : videoProviderChoice === "pixstag" ? "PixStag model" : "Seedance model / endpoint ID"}</span><input value={provider.videoModel} placeholder={videoProviderChoice === "dashscope" ? "wan3.0-video" : videoProviderChoice === "pixstag" ? "MiniMax-H3" : videoProviderChoice === "plan" ? "doubao-seedance-2.0" : "doubao-seedance-2-0-260128"} onChange={(e)=>set({videoModel:e.target.value})}/></label><label className="field"><span>Session API key</span><input type="password" value={provider.videoApiKey} placeholder={status.video?.configured ? "Loaded securely from .env.local" : "Optional session override"} onChange={(e)=>set({videoApiKey:e.target.value})}/></label></div><label className="field"><span>Video resolution</span><select value={provider.videoResolution} onChange={(e)=>set({videoResolution:e.target.value})}>{(videoProviderChoice === "pixstag" ? [["720p","720P"],["1080p","1080P"],["2k","2K"]] : [["480p","480p"],["720p","720p"],["1080p","1080p"]]).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select><small>Providers fall back to a lower tier automatically when a resolution is rejected.</small></label><label className="field"><span>Parallel video jobs</span><input type="number" min="1" max="4" step="1" value={provider.videoConcurrency} onChange={(e)=>set({videoConcurrency:Math.max(1,Math.min(4,Number(e.target.value)||1))})}/><small>2 is recommended. Ark video generation is asynchronous and may take several minutes per clip.</small></label><button className="ghost test-button" onClick={()=>testConnection("video")} disabled={!!testing}>{testing === "video" ? "Testing…" : "Test video provider"}</button>
