...
function BroadcastHeadlineOverlay({ headlineText, headlinePosition, subtitleStyle }:any) {
  ...
  const headlineFontSize = Math.max(8, 9 * 1.15 * style.fontScale / 100);
...
function ExportPanel({ title, productionMode, shots, approved, duration, audioName, bgm, build, previewUrl, downloadUrl, videoBuilds, deleteBuild, downloadResolution, setDownloadResolution, screenRatio, busy, buildProgress }: any) {
  const longScenes = productionMode === "long-scenes";
